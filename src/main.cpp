...
#if defined(NDEBUG)
#error "1Node cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) nSubsidy = 250000 * COIN;
    else if (nHeight > 0 && nHeight < 2000) nSubsidy = 1 * COIN;
    else if (nHeight >= 2000 && nHeight < 5000) nSubsidy = 4.3 * COIN;
    else if (nHeight >= 5000 && nHeight < 7000) nSubsidy = 1 * COIN;
    else if (nHeight >= 7000 && nHeight < 10000) nSubsidy = 2.85 * COIN;
    else if (nHeight >= 10000 && nHeight < 12000) nSubsidy = 1 * COIN;
    else if (nHeight >= 12000 && nHeight < 15000) nSubsidy = 2.6 * COIN;
    else if (nHeight >= 15000 && nHeight < 17000) nSubsidy = 1 * COIN;
    else if (nHeight >= 17000 && nHeight < 20000) nSubsidy = 2.5 * COIN;
    else if (nHeight >= 20000 && nHeight < 30000) nSubsidy = 1.5 * COIN;
    else if (nHeight >= 30000 && nHeight < 50000) nSubsidy = 2 * COIN;
    else if (nHeight >= 50000 && nHeight < 60000) nSubsidy = 2.5 * COIN;
    else if (nHeight >= 60000 && nHeight < 70000) nSubsidy = 3 * COIN;
    else if (nHeight >= 70000 && nHeight < 80000) nSubsidy = 3.5 * COIN;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (block.IsProofOfStake()) {
            ...
            CBitcoinAddress address = CBitcoinAddress("XVyTViUhoitnAQmqy1zd2FrCeQoUtg28wM");
            CAmount nExpectedTreasuryFunds = GetBlockValue(chainActive.Tip()->nHeight) * 0.07;
            if (block.vtx[1].vout[i - 1].scriptPubKey != GetScriptForDestination(address.Get())) {
                LogPrintf("CheckBlock(): could not verify Treasury Funds - invalid scriptPubKey\n");
                return state.DoS(100, error("CheckBlock() : Invalid Dev Funds payment"));
            }

            if (block.vtx[1].vout[i -1].nValue != nExpectedTreasuryFunds) {
                LogPrintf("CheckBlock(): could not verify Treasury Funds - invalid amount nExpectedAmount=%d nValue=%d\n", nExpectedTreasuryFunds, block.vtx[1].vout[i -1].nValue);
                return state.DoS(100, error("CheckBlock() : Invalid Dev Funds payment"));
            }
        }
    }
