...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::XON)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XON")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymize1NodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymize1NodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymize1NodeAmount = strAnonymize1NodeAmount.remove(strAnonymize1NodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymize1NodeAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymize1NodeAmount * COIN) nMaxToAnonymize = nAnonymize1NodeAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymize1NodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymize1NodeAmount));
        strAnonymize1NodeAmount = strAnonymize1NodeAmount.remove(strAnonymize1NodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymize1NodeAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
